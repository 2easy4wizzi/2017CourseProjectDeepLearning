...
class TextCNNRNN(object):
    def __init__(self, embedding_mat, non_static, hidden_unit, sequence_length, max_pool_size,
        ...
        with tf.device('/cpu:0'), tf.name_scope('embedding'):
            if not non_static:
                weight = tf.constant(embedding_mat, name='W')
            else:
                weight = tf.Variable(embedding_mat, name='W')
            self.embedded_chars = tf.nn.embedding_lookup(weight, self.input_x)
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope('conv-maxpool-%s' % filter_size):
                ...
                conv = tf.nn.conv2d(emb_pad, weight, strides=[1, 1, 1, 1], padding='VALID', name='conv')
        ...
        lstm_cell = contrib.rnn.GRUCell(num_units=hidden_unit)

        # lstm_cell = tf.nn.rnn_cell.DropoutWrapper(lstm_cell, output_keep_prob=self.dropout_keep_prob)
        lstm_cell = contrib.rnn.DropoutWrapper(lstm_cell, output_keep_prob=self.dropout_keep_prob)
        ...
        outputs, state = contrib.rnn.static_rnn(lstm_cell, inputs, initial_state=self._initial_state,
