...
from bi import load_emb
from bi import EMB_FILE_PATH
from bi import print_graph
...
LINE_FROM_CLASS = 100000
MINIMUM_ROW_LENGTH = 25
MAXIMUM_ROW_LENGTH = 150
BASE_REGULAR = '5way_tur_ger_rus_fra_usa100K_{}-{}'.format(MINIMUM_ROW_LENGTH, MAXIMUM_ROW_LENGTH)
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            def train_step(x_batch, y_batch):
                ...
                _, _, l_loss, l_acc = sess.run([train_op, global_step, cnn_rnn.loss, cnn_rnn.accuracy], feed_dict)
                return l_loss, l_acc
            ...
            for train_batch in train_batches:
                ...
                if current_step % params['evaluate_every'] == 0:
                    ...
                    if current_step % PRINT_CLASSES_STATS_EACH_X_STEPS == 0:
                        ...
                        print("temp run time(from last eval): {:0>2}:{:0>2}:{:0>2}".format(int(hours), int(minutes), int(seconds)))
            ...
            if RUN_TEST_AFTER_TRAIN:
                ...
                tst_loss_total, tst_acc_total = [tst_loss_total] * len(dev_acc_over_steps), [(float(total_test_correct) / float(len(y_test)))] * len(dev_acc_over_steps)
                print(acc_msg.format(total_test_correct, len(y_test), my_acc))

                print_graph('loss/epochs(train in red, validation in green, test(constant) in blue)', 'epochs', 'loss', trn_loss_over_steps, dev_loss_over_steps, tst_loss_total)
                print_graph('acc/epochs(train in red, validation in green, test(constant) in blue)', 'epochs', 'acc', trn_acc_over_steps, dev_acc_over_steps, tst_acc_total)
